...
List<UserProfileData> filterCity(
    List<UserProfileData> users, UserProfileData currentUser) {
  return users.where((user) => user.location == currentUser.location).toList();
}

List<UserProfileData> reranking(
    List<UserProfileData> users, UserProfileData currentUser) {
  ...
  users.sort((a, b) {
    ...
    final Set<dynamic> currentUserTracks =
        (currentUser.spotifyData?.favoriteTracks ?? {}).toSet();
    ...
    double overlapA =
        aTracks.intersection(currentUserTracks).length / currentUserTrackCount;
    double overlapB =
        bTracks.intersection(currentUserTracks).length / currentUserTrackCount;
...
List<UserProfileData> matchUsers(
    List<UserProfileData> users, UserProfileData currentUser) {
